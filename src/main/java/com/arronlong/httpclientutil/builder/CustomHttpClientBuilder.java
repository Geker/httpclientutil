...
public class CustomHttpClientBuilder extends HttpClientBuilder {
	 ...
	 * @return 返回当前对象
	 */
	public CustomHttpClientBuilder timeout(int timeout) {
	 ...
	 * @return 返回当前对象
	 */
	public CustomHttpClientBuilder timeout(int timeout, boolean redirectEnable) {
		// 配置请求的超时设置
		RequestConfig config = RequestConfig.custom()
				...
				.build();
		return (CustomHttpClientBuilder) this.setDefaultRequestConfig(config);
	}
	 ...
	 *             http处理异常
	 */
	public CustomHttpClientBuilder ssl() throws HttpProcessException {
...
//		return (HCB) this.setConnectionManager(connManager);
		return (CustomHttpClientBuilder) this.setSSLSocketFactory(ssls.getSSLCONNSF(sslpv));
	}
	 ...
	 *             http处理异常
	 */
	public CustomHttpClientBuilder ssl(String keyStorePath) throws HttpProcessException {
	 ...
	 *             http处理异常
	 */
	public CustomHttpClientBuilder ssl(String keyStorePath, String keyStorepass) throws HttpProcessException {
...
//		this.isNewSSL=true;
	 ...
	 *             http处理异常
	 */
	public CustomHttpClientBuilder pool(int maxTotal, int defaultMaxPerRoute) throws HttpProcessException {
		Registry<ConnectionSocketFactory> socketFactoryRegistry = RegistryBuilder
				.<ConnectionSocketFactory> create()
				.register("http", PlainConnectionSocketFactory.INSTANCE)
		...
		isSetPool=true;
		return (CustomHttpClientBuilder) this.setConnectionManager(connManager);
	}
	 ...
	 * @return 返回当前对象
	 */
	public CustomHttpClientBuilder proxy(String hostOrIP, int port) {
		// 依次是代理地址，代理端口号，协议类型
		HttpHost proxy = new HttpHost(hostOrIP, port, "http");
		DefaultProxyRoutePlanner routePlanner = new DefaultProxyRoutePlanner(proxy);
		return (CustomHttpClientBuilder) this.setRoutePlanner(routePlanner);
	}
	 ...
	 * @return 返回当前对象
	 */
	public CustomHttpClientBuilder retry(final int tryTimes) {
	 ...
	 * @return 返回当前对象
	 */
	public CustomHttpClientBuilder retry(final int tryTimes, final boolean retryWhenInterruptedIO) {
		// 请求重试处理
	 ...
	 * @return	返回当前对象
	 */
	public CustomHttpClientBuilder sslpv(String sslpv) {
	 ...
	 * @return	返回当前对象
	 */
	public CustomHttpClientBuilder sslpv(SSLProtocolVersion sslpv) {
		this.sslpv = sslpv;
		return this;
	}
}