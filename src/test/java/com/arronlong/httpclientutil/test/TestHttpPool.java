...
public class TestHttpPool {
	...
	private static void testByPool(int getCount, int downCount) throws HttpProcessException {
		...
		CustomHttpClientBuilder hcb = CustomHttpClientBuilder.custom().pool(100, 10).timeout(10000).ssl();
		if(getCount>0){
	...
	public static void testquickConcurrent() throws HttpProcessException{
		...
		HttpClient client = CustomHttpClientBuilder.custom().pool(100, 20).timeout(10000).build();// 最多创建20个http链接
